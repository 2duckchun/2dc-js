...
export default function Lecture() {
  return (
    <div className={styles.act_container}>
      ...
      ></ActNavigator>
      <h1>연산자(Operator)</h1>
      <p>본격적인 여행을 떠나기 전에 필수적으로 알아야할 연산자라는 개념에 대해 짚고 넘어갑시다.</p>
      <p>연산자는 무엇이고, 연산자를 어떻게 사용할까요?</p>
      <p>사실 우리는 이전 섹션에서도 연산자를 계속 사용하고 있었습니다.</p>
      <h2>연산자란?</h2>
      <p>연산이란 특정한 작업을 수행해서 값을 나타내는 것을 의미합니다.</p>
      <p>
        이런 뜻에서 자바스크립트의 연산자는 <b>특정한 값을 통해 다른 값을 산출하는 명령어</b> 라고
        할 수 있겠습니다.
      </p>
      <p>5 + 5를 자바스크립트를 통해 연산한다고 가정해보겠습니다.</p>
      <p>
        우리가 쓴 5, +, 5는 모두 기계어로 변환되어 CPU에서 계산될 것입니다. 그리고 값이 산출될
        것입니다.
      </p>
      <p>
        여기서 5는 연산을 당하는 주체인 <b>피연산자</b>라고 하고, 두 피연산자를 연산하고자 지시하는
        명령어인 +를 <b>연산자</b>라고 합니다.
      </p>
      ...
      <p>할당 연산자는 너무 쉽습니다.</p>
      <p>
        이퀄 기호인 <code className={styles.code_tag}>=</code> 이 할당 연산자(assignment
        operator)라고 합니다.
      </p>
      <p>
        할당 연산자는 <b>우측의 값</b>을 <b>좌측의 변수</b>에 그대로 할당하는 역할을 합니다.
      </p>
      <p>
        즉, =은 ~와 같다가 아니라 <b>우변을 좌변에 할당한다.</b> 가 맞겠습니다.
      </p>
      <p>+=, -=, /= 같이 생긴 할당 연산자도 존재합니다. 분명히 사용할 일이 있으실텐데요.</p>
      <p>할당 연산자의 편의 기능이므로 나중에 따로 공부해보시는 것을 추천합니다!</p>
      ...
      <p>
        1과 2는 각각 떨어져 있으면 단순한 값의 나열일 뿐이지만 +라는 연산자를 만나면 3이라는 값으로
        연산됩니다.
      </p>
      <p>여기서 잠시 표현식이라는 단어를 알아볼까요?</p>
      <p>
        표현식은 <b>값을 생성하거나, 값을 표현하는 코드의 구조 자체</b>를 의미합니다.
      </p>
      <p>
        1 + 2는 표현식일까요? <b>네 맞습니다!</b> 3이라는 값을 생성하고 표현할 수 있습니다.
      </p>
      <p>
      ...
      <p>
        <b>어떤 값으로 표현될 수 있는 코드 자체</b>를 <b>표현식</b>이라고 합니다.
      </p>
      <p>자 넘어가봅시다!</p>
      <h2>이항 산술 연산자</h2>
      <p>이름은 거창하지만 별거 없는 이항 산술 연산자입니다.</p>
      <p>
        이항 산술 연산자(Binary arithmetic operators)는 초등학교때 배우는 덧셈, 뺄셈, 나눗셈, 곱셈과
        같은 산술 연산을 하는 기호입니다.
      </p>
      <p>두개의 값을 하나의 표현식으로 만드는 역할을 하지요.</p>
      <CodeBox code={lessonCode.arithmetic} />
      <p>1 + 2의 연산값 3입니다.</p>
      <p>사실 위의 표현식은 어떤 변수에 할당하지 않았으므로 두고두고 쓰기가 어렵습니다.</p>
      <p>변수에 할당하지 않았으므로 저장된 메모리 주소를 알 수 없기 때문이죠.</p>
      <p>하지만 표현식 자체는 값 자체로 이용할 수 있습니다. 아래 코드를 보시죠.</p>
      <CodeBox code={lessonCode.adding1Function} />
      <br />
      <p>함수입니다. 아직 함수의 개념을 배우지는 않았지만 익숙해지시는 차원에서 한번 보시죠!</p>
      <p>
        함수의 괄호친 곳에 5 + 5가 파라미터(매개변수)라는 것으로 들어있는 것을 보실 수 있습니다.
      </p>
      <p>5라는 피연산자들은 +에 의해 연산되어 하나의 표현식이 되기 때문에 10으로 평가되고</p>
      <p>최종적으로 const b는 11이 할당 됩니다.</p>
      <h3>단항 산술 연산자의 종류</h3>
      <CodeBox code={lessonCode.otherArithmetic} />
      <p>덧셈뺄셈곱셈나눗셈은 익숙하시겠지만 나머지 연산자는 처음 보시는 분이 꽤 되실겁니다.</p>
      <p>
        나머지 연산자는 <b>모듈러 연산</b>이라고도 하는데요. 정수론에서 중요한 개념 중 하나입니다.
      </p>
      <p>
        풀어보자면, <b>A mod B = C은 B로 A를 나눈 나머지가 C와 같다</b>는 뜻 입니다.
      </p>
      <p>
      ...
      <details>
        ...
        <br />
        <p>나머지 연산자는 의도적인 연습을 몇번 해보시면 금방 익힐 수 있습니다.</p>
        <p>카운트 6마다 실행되는 변수를 만들때 count % 6 === 0 과 같이 표현할 수 있듯이</p>
        <p>모듈러 연산은 활용 방법이 참 많습니다.</p>
      </details>
      <h2>단항 산술 연산자</h2>
      <p>단항 산술 연산자는 1개의 피연산자를 산술하여 표현식을 만드는 연산자입니다.</p>
      <h3>단항 증감 연산자</h3>
      <p>
        ++, -- 증감 연산자는 피연산자의 값을 증가 또는 감소시키고 값을 변경하는 부수 효과(side
        Effect)가 있습니다.
      </p>
      <p>값 자체가 변경된다는 것이 중요합니다!</p>
      <p>아래 코드를 구글 크롬 콘솔에 한번 쳐보세요!</p>
      <CodeBox code={lessonCode.unaryIncreaseDecrease} />
      <p>최종적인 a의 값은 같지만, console에 즉시 나오는 a의 값은 뒤죽박죽일 것입니다.</p>
      <p>
        증감 연산자가 변수의 앞에 있느냐 (prefix냐 postfix냐)에 따라 값의 실행 순서가 바뀌게되어
        위와 같은 현상이 나타납니다.
      </p>
      <p>코드를 깔끔하게 작성하기 위해 알고 넘어가시면 좋습니다.</p>
      <p>
        물론 당장 알 필요까지는 없을 것 같지만, 원리에 대해 궁금하면 mdn 문서에서 찾아보도록 합시다!
      </p>
      <p>++, --의 prefix, postfix 부분을 찾아보시면 됩니다!</p>
      <h3>단항 기호 연산자</h3>
      <p>
        단항 기호(+, -) 연산자는 피연산자를 숫자로 변환하는 기능을 가지고 있습니다. 부수효과는
        없습니다.
      </p>
      <p>사용할 일이 많지는 않을 연산자 같습니다.</p>
      <p>단순히 값을 강제로 숫자로 만드는 연산자구나~ 라고 생각하고 넘어가셔도 좋습니다.</p>
      <br />
      <p>
        <b>&quot;+ 단항 연산자&quot;</b> 는 피연산자를 숫자로 변환하는 기능외에 별다른 기능을 가지고
        있지 않습니다.
      </p>
      ...
      <p>
        <b>&quot;- 단항 연산자&quot;</b> 는 피연산자를 숫자로 변환하면서 숫자의 부호를 바꾸는 기능을
        합니다.
      </p>
      ...
      <p>비교 연산자는 뭘까요?</p>
      <p>
        <b>황소가 검은소보다 일을 잘하느냐?</b>라는 질문에 농민은 <b>어유 그런 말씀 마세유</b> 라는
        말을 할 수 있지만
      </p>
      <p>
        컴퓨터는 동일한 질문에 <b>맞다(true), 틀리다(false)</b> 두가지 답변만 해줄 것입니다.
      </p>
      <p>
      ...
      <p>
        동등하다와 일치하다가 같은 뜻으로 보이실 수 있습니다. 비슷하긴 하지만 다른 개념입니다.
        코드를 통해 살펴보겠습니다.
      </p>
      ...
      <p>
        연산할 피연산자의 타입이 다르면 에러를 낼 법도 한데 자바스크립트는 어떻게든 연산을 해내려고
        하는데요.
      </p>
      ...
      <p>즉 약타입이라는 이유 (타입에 매우 관대함) 때문에 동등 비교와 일치 비교가 나뉘게 됩니다.</p>
      <p>
        <b>동등 비교</b>에서는 동등한지 아닌지 판별하기 위해서 강제적으로 형 변환을 한 뒤에 일치
        여부를 비교합니다.
      </p>
      <p>
        <b>일치 비교</b>는 자동 형변환 없이 데이터의 값이 완벽히 일치하는지 비교합니다.
      </p>
      <p>
        동등비교는 상당히 난잡한 상황을 만들 수 있으므로 대부분의 상황에서는 일치 비교를 쓰시는 편이
        낫습니다.
      </p>
      <h3>부동등 불일치 비교</h3>
      ...
      <h2>논리 연산자</h2>
      <p>
        논리 연산자는 논리 연산 기호를 이용해서 좌항과 우항의 논리값을 판단합니다. (따라서 true,
        false만 반환합니다.)
      </p>
      <p>
        논리 연산자는 뭔가를 논증할때 사용하는 <b>또는(OR)</b>, <b>그리고(AND)</b>와 동일한데요.
      </p>
      <p>
        <b>OR은 비교 대상이 하나만 참(true)이어도 전체가 참(true)입니다.</b>
      </p>
      <p>
        <b>AND는 비교 대상 중 하나만 거짓(false)이어도 전체가 거짓(false)입니다.</b>
      </p>
      <p>
        자바스크립트에서 OR은 <b>논리합(||)</b>이라고 하며, AND는 <b>논리곱(&&)</b>이라고 합니다.
      </p>
      <p>코드를 통해 살펴보겠습니다.</p>
      ...
      <p>
        피연산자는 불리언값이 아니어도 됩니다. 논리 연산자는 피연산자의 타입을 암묵적으로
        boolean값으로 변형시킵니다.
      </p>
      <p>약타입 만세입니다.</p>
      ...
      <p>
        <b>이런 상황을 truthy하다, falsey하다로 나누는데,</b> 이것은 다음 섹션에서 배워보도록
        하겠습니다.
      </p>
      <h2>이 외에도...</h2>
      <p>자바스크립트 연산자는 생각보다 많습니다.</p>
      <p>연산은 새로운 표현식을 만드는 방법이기 때문인데요.</p>
      <p>피연산자를 대상으로 다른 값을 산출해내면 그것을 연산자라고 할 수 있습니다.</p>
      <p>
      ...
      <p>
        연산자가 많아서 연산자 우선순위 종류도 많은데 일단 사칙연산이다 정도로만 알고 넘어가시죠!
      </p>
      <br />
      <p>이번 챕터에서는 연산자의 정의와 필수적인 연산자만 알아가셔도 성공입니다.</p>

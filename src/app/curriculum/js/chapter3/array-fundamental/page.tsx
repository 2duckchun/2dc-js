...
export default function Lecture() {
  return (
    <div className={styles.act_container}>
      ...
      <p>
        본격적으로 배열을 배우기 전에 데이터의 원시 타입과 참조 타입의 동작 원리에 대해 다뤄볼
        것입니다.
      </p>
      <p>
        그리고 배열의 특수한 기능들과, 그 기능들을 사용할 수 있게 해주는 패러다임인 프로토타입에
        대해 배웁니다.
      </p>
      <p>우리가 자주 사용할 배열 전용 프로토타입 함수들을 직접 만들어보는 시간도 가질 것입니다.</p>
      <p>차근차근 알아가 보겠습니다!</p>
      <h2>배열이란 무엇인가?</h2>
      <p>배열이란 무엇일까요?</p>
      <p>
        컴퓨터 과학에서는 배열은 인덱스와 인덱스에 대응하는 데이터들로 이루어진 자료구조 라고
        명시합니다.
      </p>
      <p>
        위의 말을 간단히 하면,{' '}
        <b>여러 데이터들에게 번호표(인덱스)를 하나씩 나눠준 뒤 일렬로 줄 세워 묶어버린 것</b>이
        배열입니다.
      </p>
      <p>즉, 배열을 이용하면 여러개의 데이터를 한번에, 규칙적으로 사용할 수 있습니다.</p>
      <h2>원시 타입과 참조 타입</h2>
      <p>배열로 여행을 떠나기 전에 데이터 타입을 조금만 공부해보고 넘어가겠습니다.</p>
      <p>우리는 아래 코드의 조건식이 true임을 알고 있습니다.</p>
      <CodeBox code={lessonCode.primitiveCondition} />
      <br />
      <p>
        그런데 이 코드들의 조건식은 왜 false일까요? 위의 코드와 별로 달라보이는게 없는데 말입니다.
      </p>
      ...
      <p>
        ...
        <b>저장된 값은 변경되지 않는다(불변성을 지닌다)</b>는 특징을 가진 데이터 타입들입니다.
      </p>
      ...
      <p>
        숫자 25가 메모리에 기록되고 식별자가 메모리 주소를 참조하는 것을 대략적으로 그려보았습니다.
      </p>
      <br />
      ...
      />
      <p>
        숫자형 데이터 25는 메모리 특정 공간에 기록이 되고, 그 메모리 공간의 주소를 식별자가 참조하고
        있습니다.
      </p>
      <br />
      ...
      <p>b가 a를 참조했으니, b는 a의 메모리 주소를 그대로 참조하고 있을까요??</p>
      <p>정답은 그렇지 않다 입니다.</p>
      <p>
        숫자형 데이터는 원시 타입이기 때문에 메모리 주소가 아니라 값 자체를 복제한 후, 별도의
        메모리에 저장합니다.
      </p>
      ...
      <p>자바스크립트의 원시 타입은 불변성이기 때문에 메모리에 저장된 값이 바뀌지 않습니다.</p>
      <p>
        그 대신 메모리의 다른 부분에 30이 저장되고, 변수 a는 30이 저장된 메모리의 주소를 다시
        참조합니다.
      </p>
      ...
      <p>
        어떤 변수에도 참조되지 않는 메모리는 <b>가비지 컬렉터</b>에 의해 회수됩니다.
      </p>
      <p>
        다만 가비지 컬렉터는 언어의 내부적 동작에 의해 실행되므로, 메모리의 회수 시기는 우리가 알 수
        없습니다.
      </p>
      <br />
      <p>
        이렇게 메모리를 알아서 관리해주는 언어를 <b>매니지드 언어</b>라고 합니다.
      </p>
      <p>
      ...
      <p>b에 새로운 표현식이 할당되었으므로, 표현식이 계산된 값이 메모리에 할당 될 것입니다.</p>
      ...
      <ul>
        <li>
          <b>변수에 원시타입 데이터를 할당할때는 늘 새로운 값이 할당된다는 것과</b>
        </li>
        <li>
      ...
      <p>원시 타입 데이터 변수 선언을 실행 컨텍스트의 관점에서 이해해보겠습니다.</p>
      <p>함수가 실행되면 실행 컨텍스트가 생성되고, 변수들이 환경 레코드에 등록됩니다.</p>
      <p>실행 컨텍스트의 변수 등록을 메모리 관점에서 생각해보면 아래와 같을 것입니다.</p>
      <CustomImageWithCaption
        imageSrc={primitive5}
        width={643}
        height={355}
      ...
      <p>
        실제로 자바스크립트 식별자가 바라보는 관점에서 a와 b는 동일한 값을 가지고 있으므로 두 비교는
        참입니다.
      </p>
      <p>
        <b>그런데 c와 d는 무엇이 다를까요?</b> 둘다 [] 라는 값을 가지고 있는데 말입니다.
      </p>
      <br />
      <p>
        <b>[]</b>는 <b>배열(Array)</b>이라는 데이터 구조이고, 참조 타입의 데이터 입니다.
      </p>
      ...
      <p>데이터를 담을 수 있다는 것을 표현하고자 product와 cart을 배열로 만들었습니다.</p>
      ...
      <p>
        heap이라는 메모리의 가변적인 영역에 데이터가 할당되고, 변수는 heap의 메모리 주소를 참조하게
        됩니다.
      </p>
      <p>
        즉, 변수에 할당된 값이 다르므로, 둘을 비교 연산하면 false가 나오게 되는 것 입니다. ( 위
        예시의 0x2345와 0x3516이 다르듯이요!)
      </p>
      <br />
      ...
      <p>
        이 경우, product의 참조 주소값을 cart가 그대로 가져오므로 두 식별자는 같은 배열을 참조하게
        됩니다.
      </p>
      <br />
      <CustomImageWithCaption
      ...
      <p>즉, 둘은 같은 배열을 참조하고 있으므로 첫번째 비교에서는 true가 출력됩니다.</p>
      <br />
      <p>이후 product에 새로운 배열이 할당이 되었습니다.</p>
      <p>이제 product는 다른 배열을 참조하게 됩니다.</p>
      <p>
        다만 cart는 여전히 이전에 생성된 배열 주소를 참조하고 있으므로, 둘을 비교하면 false가
        출력됩니다.
      </p>
      <br />
      <hr />
      ...
      <p>
        다만, 개발 중 알 수 없는 현상이 일어났을 때, 그것을 캐치하고 해결하려면 언어의 내부 원리를
        어느정도는 알고 있어야 합니다.
      </p>
      <p>이제 원시 타입 데이터 종류와 참조 타입 데이터 종류를 알아보겠습니다.</p>
      ...
      <p>
        앞서 말씀드렸듯이 배열은 특정 데이터들을 인덱스 순서대로 묶을 수 있는 기능을 하는 데이터
        타입입니다.
      </p>
      ...
      <p>
        배열 내부에 있는 각각의 데이터를 <b>엘리먼트</b>라고 하며, 배열 내 데이터에 접근할 수 있는
        숫자를 <b>인덱스</b>라고 합니다.
      </p>
      ...
      <br />
      <p>그리고 배열 내 엘리먼트에 바로 접근할 수 있는 숫자를 인덱스라고 합니다.</p>
      <p>인덱스는 0부터 시작해서 배열 길이 - 1으로 끝납니다.</p>
      <p>우리의 과일바구니는 4개의 엘리먼트가 있었죠.</p>
      <p>즉, 과일바구니의 배열 길이는 4이고, 배열의 최대 인덱스는 3입니다.</p>
      <p>
        배열에 인덱스로 접근해서 엘리먼트를 가져오는 문법은 <b>배열[인덱스숫자]</b> 입니다.
      </p>
      <p>
        아래의 코드가 이해가 잘 가신다면 인덱스와 엘리먼트에 대한 기초적인 이해는 모두 되었다고 봐도
        무방합니다.
      </p>
      <CodeBox code={lessonCode.arrayExplain4} />
      <br />
      <p>위의 코드에서는 대괄호 내부에 숫자가 오고 있다는 것을 기억해주시기 바랍니다.</p>
      <p>
        다만 인덱스를 벗어나는 숫자가 오게 될 경우, 배열 내 값이 없다는 의미로 <b>undefined</b>가
        출력됩니다.
      </p>
      ...
      <p>프로퍼티는 객체의 용어이고, 객체 챕터에서 더 자세하게 다룰 주제입니다.</p>
      <p>
        자바스크립트에서 배열은 객체의 일부이고, 그렇기 때문에 별도의 프로퍼티를 가지고 있습니다.
      </p>
      <br />
      <p>
        length는 배열의 길이를 나타내는 숫자형 데이터로, 배열에 .length를 붙여 입력하면 배열의
        길이를 알 수 있습니다.
      </p>
      <p>배열의 최대 인덱스는 length - 1임을 꼭 기억해주세요! 굉장히 많이 사용됩니다!</p>
      <h2>Exercise</h2>
      <ol>
        <ExerciseCard
      ...
      <p>
        다음 장에서는 배열을 다루는데 필수적으로 알아야 하는 배열 메서드와, 메서드를 사용할 수 있게
        해주는 프로토타입에 대해 배워보겠습니다.
      </p>
      <p>고생하셨습니다!</p>
    </div>
  );

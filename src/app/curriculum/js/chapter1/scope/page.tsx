...
export default function Chap0Act1() {
  return (
    <div className={styles.act_container}>
      ...
      <p>프로그래밍 언어에서 스코프는 굉장히 중요한 개념입니다.</p>
      <p>
        코드 내에서 변수를 어떻게 참조할 것인지, 식별자를 어떤 식으로 분리할
        것인지를 스코프라는 개념을 통해 나누기 때문인데요.
      </p>
      <p>
        앞서 변수 파트와 제어문 파트에서 var와 let으로 살살 장난을 쳤었던 그
        스코프!
      </p>
      <p>여기서 그 개념을 확실히 알아가도록 합시다.</p>
      <h2>스코프란 무엇인가?</h2>
      <p>
      ...
      <p>이러한 현상을 스코프 체이닝이라고 합니다.</p>
      <p>만약 글로벌 스코프에도 선언된 변수가 없다면 참조 에러를 출력합니다.</p>
      ...
      <br></br>
      <p>
        그렇다면, 앞서 말했던 나온 글로벌 스코프, 로컬 스코프, 스코프 체이닝,
        블록 스코프 등등은 뭘까요?
      </p>
      ...
      <h2>스코프 체이닝</h2>
      <p>
        스코프가 변수의 참조 공간이라 하더라도 아주 별개로 나뉘어 있는 공간은
        아닙니다.
      </p>
      <p>스코프는 식별자의 유효 범위라고 말씀을 드렸었죠!</p>
      <p>
        만약 하위 스코프에서 어떤 변수를 참조했는데, 일치하는 식별자가 없을 경우
      </p>
      <p>자신의 상위 스코프에 식별자가 있는지 확인을 합니다.</p>
      ...
      <p>
        하위 스코프에서 어떤 변수를 참조하고 있지만 선언된 변수가 없다면, 하위
        스코프는 상위 스코프에서 참조할만한 변수를 찾습니다.
      </p>
      <br></br>
      ...
      <p>
        ...
        if, for, while문은 블록 레벨 스코프를 가집니다.
      </p>
      <p>
      ...
      <p>아래는 블록 레벨 스코프를 가진 let을 사용한 for문입니다.</p>
      <br></br>
      ...
      <p>명심하세요! var는 오로지 함수와 사용했을때만 안전합니다.</p>
      <br></br>
      ...
      <p>참조가 끊긴 변수는 가비지 컬렉터에 의해 메모리에서 회수됩니다.</p>
      <p>
        (함수의 경우에는 <b>클로저</b>라는 약간 특이한 현상이 있습니다. 다음
        챕터에서 깊게 다뤄볼 주제입니다!)
      </p>
      <br></br>
      <p>
      ...
      <p>
        다음 챕터에서는 자바스크립트의 핵심인 <b>함수</b>를 배워보겠습니다.
      </p>
      <h2>Exercise</h2>
      <ol>
        <ExerciseCard

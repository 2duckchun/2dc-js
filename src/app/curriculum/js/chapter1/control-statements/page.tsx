...
export default function LessonPage() {
  return (
    <div className={styles.act_container}>
      ...
      <p>
        하지만 코드가 정해진 방향과 순서대로만 동작하게 된다면 우리는 코드를
        통해 기능을 추가하기가 굉장히 어렵겠죠.
      </p>
      <p>
      ...
      <p>
        이것을 코드 블록이라고 하지요. 코드 블록은 코드를 묶는 것 외에
        <b> 스코프(scope) </b>라는 것을 생성하는 역할도 합니다.
      </p>
      <p>
        스코프는 범위라는 뜻을 가지고 있습니다. 자세한 이야기는 다음 챕터인
        스코프에서 다룰 예정이지만, 여기서 간단히 다루고 넘어가보도록
        하겠습니다.
      </p>
      ...
      <p>let이라는 변수 선언 키워드가 블록 레벨 스코프를 가지기 때문입니다.</p>
      <p>
      ...
      <p>
        var는 함수 레벨의 스코프를 가지므로, 함수 스코프 내에서 선언된 var만
        유일할 수 있습니다.
      </p>
      <p>
      ...
      <p>
        아래 코드 블록은 이번 학습에 필요한 자바스크립트의 데이터 타입 목록을
        간추려놓은 것입니다.
      </p>
      <CodeBox code={lessonCode.dataType} />
      <p>
        특정 데이터가 조건절에 할당되면 자바스크립트 엔진은 조건절에 할당된 값이
        truthy인지 falsy인지 자동적으로 평가합니다.
      </p>
      <p>
        즉 조건절의 분기는 true, false에 의해 나뉘는 것이 아니라, truthy,
        falsy에 의해 나뉘게 됩니다.
      </p>
      <p>데이터 타입의 값마다 truthy, falsy를 평가하는 기준이 약간 다른데요.</p>
      <p>
        다행히 외울 필요는 없습니다. 공부하다보면 자연스레 익히게 되는 부분이며,
        falsy 부분 외에는 모두 truthy로 해석해도 무리가 없습니다.
      </p>
      <p>아래 표를 참고해주세요!</p>
      <table
        width="100%"
      ...
      <p>
        추가로 다른 조건들을 걸고 싶다면 else if를 사용할 수 있습니다. 이때 else
        if는 한 조건문에 여러개가 있을 수 있지만 if문과 else문은 하나만 있어야
        합니다.
      </p>
      <p>모든 조건들이 falsy 하다면 else의 코드 블록이 실행됩니다.</p>
      <CodeBox code={lessonCode.ifExercise} />
      <p>팁을 드리자면 if문에 꼭 else를 붙일 필요는 없습니다.</p>
      <p>
        if 자체만 사용할 일이 더 많으실 수도 있습니다. 직관력은 코드를
        하시다보면 점차 늘게 되실 것입니다.
      </p>
      <h2>반복문</h2>
      <h3>for</h3>
      ...
      <p>
        원리는 생각보다 단순하지만 직관적이지 않으므로 처음 배우실때는 이해가 잘
        안될수도 있습니다. 직관을 키워야 합니다.
      </p>
      <p>직관을 키우기 위해서는 의도적인 연습과 분석이 필수입니다.</p>
      ...
      <p>의도적인 연습을 많이 하시면 for문을 익히는데 도움이 될 수 있습니다.</p>
      <br></br>
      ...
      <p>while문은 반복횟수가 불명확할때 사용되는 반복문인데요.</p>
      <p>
        특정 숫자의 각 자릿수를 모두 더하는 알고리즘을 짠다고 가정해보겠습니다.
      </p>
      <p>
        (이런 문제는 코드 블록이 몇번동안 실행되어야 할지 모르기 때문입니다.)
      </p>
      <CodeBox code={lessonCode.whileAlgorithm} />
      <p>코드를 하나하나 되짚어가며 사고해보시기 바랍니다!</p>
      <h3>반복문 탈출</h3>

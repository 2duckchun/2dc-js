...
export default function Lecture() {
  return (
    <div className={styles.act_container}>
      ...
      ></ActNavigator>
      <h1>재귀 (맛보기)</h1>
      <p>실행 컨텍스트와 클로저 현상을 이해하셨으니 재귀함수도 한번 다뤄보겠습니다.</p>
      <p>일단 여기에서는 맛보기로써의 재귀함수만 다룰 예정입니다.</p>
      <p>새롭고 신비한 것을 배우신다는 느낌으로 이 챕터를 봐주셨으면 좋겠습니다.</p>
      ...
      <br />
      <p>아래는 숫자 n을 입력받아, 1부터 n까지의 모든 수를 더하는 재귀함수입니다.</p>
      ...
      <p>
        재귀 함수를 보다 쉽게 이해하시려면 함수의 실행과 콜스택의 상관관계를 머릿속에 그리면서
        접근해야 합니다.
      </p>
      <p>아래 그림은 sum1toN에 5를 인수로 넘겨줬을 때 생성될 최대 콜스택입니다.</p>
      ...
      />
      <p>
        함수 내부에서 함수가 호출되면, 호출된 함수의 실행 컨텍스트가 생성되어 콜스택에 쌓입니다.
      </p>
      <p>
        함수의 리턴값은 n + sum1toN(n-1)인데, sum1toN(n-1)이 해결이 되어야만 값을 낼 수 있습니다.
      </p>
      <p>따라서 재귀된 함수들은 콜스택에 계속 쌓입니다.</p>
      <p>n이 마침내 재귀가 아니라 1을 리턴하게 되었을 때, 콜스택이 정리되기 시작합니다.</p>
      <p>각 실행 컨텍스트 내 코드들이 하나씩 리턴되면서 최종적으로는 15가 되는 것이지요.</p>
      <p>
        콜스택은 5, 4, 3, 2, 1 순으로 표현이 되었지만, 실제적으로는 1, 2, 3, 4, 5 순으로 덧셈이
        이루어졌습니다.
      </p>
      <p>값의 실제 계산과 콜스택을 비교하여 이해하시는 것이 굉장히 중요합니다.</p>

      <h2>재귀함수의 필수조건</h2>
      <p>재귀함수는 재귀할때마다 콜스택을 하나씩 사용합니다.</p>
      <p>만약 함수가 어떤 값을 리턴하지 않고, 계속 재귀만 한다면 어떻게 될까요?</p>
      <p>콜 스택에 실행 컨텍스트가 너무 많이 쌓인 나머지, 콜스택이 터져버리는 현상이 발생합니다.</p>
      ...
      <ol>
        <li>
          <b>함수의 종료조건.</b> 어느 조건에서는 꼭 재귀가 아닌 값을 return 해야 합니다.
        </li>
        <li>
          <b>재귀할 때의 파라미터 변화.</b> 똑같은 파라미터로 똑같이 호출하면 콜스택은 무조건
          터집니다.
        </li>
      </ol>
      <br />
      <p>
        재귀함수에서, 함수의 종료조건은 베이스(base)라고 하고, 재귀조건은 말 그대로 재귀라고
        명명합니다.
      </p>
      <CodeBox code={lessonCode.recursiveExplain} />
      <h2>반복문 vs 재귀함수</h2>
      <p>재귀함수는 반복문으로 나타낼 수 있고, 반복문은 재귀함수로 나타낼 수 있습니다.</p>
      <p>1부터 n까지 console.log를 찍는 함수를 반복문과 재귀함수로 만들어 보겠습니다.</p>
      ...
      <br />
      <p>위의 두 코드는 기능면에서는 동일하지만, 효율면에서는 약간의 차이가 존재하는데요.</p>
      ...
      <p>재귀는 동일한 작업을 반복하는 큰 작업 단위를 해결할 때 큰 효과를 발휘합니다.</p>
      <br />
      <p>예를 들어 아래 코드에 명시된 객체 데이터에서 name 부분만 출력하고 싶다고 가정해봅시다.</p>
      ...
      <p>
        당장에는 감이 잘 오지 않으시겠지만, 계속 다뤄보고 공부하다보면 약간의 직관이 생기실
        것입니다.
      </p>
      <p>(저도 완벽한 직관이 생기지는 않았지만, 열심히 노력중입니다.)</p>
      <p>지금까지는 재귀 함수의 맛보기 단계 였습니다.</p>
      <p>더 심화된 재귀함수의 사용사례 및 실습은 다른 챕터에서 다룰 수 있다면 더 다뤄보겠습니다.</p>
      <p>재귀를 사용하는 몇가지 간단한 문제들을 풀어보시고, 이번 단원을 마무리하겠습니다.</p>

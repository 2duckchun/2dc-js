...
export default function Lecture() {
  return (
    <div className={styles.act_container}>
      ...
      <p>
        함수를 이해하기 위해선 다양한 내용들을 다루어야 하기 때문에, 자료를 만들면서 정말 많은
        압박을 받았습니다.
      </p>
      <p>하지만 감사하게도, 저 또한 많은 공부를 하게 되었습니다.</p>
      <p>만약 강의 자료가 너무 어렵게 느껴지거나 혼란스럽게 보인다면 언제든 코멘트를 남겨주세요!</p>
      <p>의견을 주시는 것은 제게 큰 영광입니다.</p>
      <br />
      <p>이 챕터가 여러분의 자바스크립트 여행에 큰 도움이 되었으면 하는 바람입니다.</p>
      <p>그럼 시작해봅시다!!</p>
      <br></br>
      ...
      <p>함수 기계인데요. x를 넣으면 함수 기계 내부의 식을 거쳐서 y값이 산출됩니다.</p>
      <p>
        자바스크립트에서의 함수도 약간 비슷한 개념입니다. 어떤 값을 넣으면 내부의 코드 블록을 거쳐
        값이 출력됩니다.
      </p>
      ...
      <h2>자바스크립트에서 함수란?</h2>
      <p>
        자바스크립트 함수의 정의는 <b>코드 실행 단위를 묶어 나중에 호출할 수 있게 하는 것</b>으로
        내릴 수 있습니다.
      </p>
      <p>나중에 호출할 코드 덩어리를 만들수 있다는 것은 프로그래밍적으로 엄청난 이점이 있습니다.</p>
      <p>코드 덩어리들을 함수로 묶어서 사용하는 것의 장점은 아래와 같습니다.</p>
      <h2>적절한! 함수 사용의 장점</h2>
      <ol>
        <li>재사용성 : 필요할 때마다 호출할 수 있다.</li>
        <li>유지보수성 : 로직을 수정해야 할 경우, 함수만 수정하면 된다.</li>
        <li>가독성 : 함수명을 잘 지으면 코드만 보고도 기능을 유추할 수 있다.</li>
      </ol>
      <h3>재사용성</h3>
      <p>
        함수는 코드의 블록을 캡슐화하는데 사용되므로, 비슷한 작업을 반복적으로 수행할 때 굉장히
        편리합니다.
      </p>
      <p>아래의 값들을 함수 없이 구했다면 어땠을지 상상해보세요!</p>
      <CodeBox code={lessonCode.reuseability} />
      <p>
        위의 코드에서는 더하기라는 로직을 함수화하여 재사용을 하고 있습니다. 함수는 재사용성이
        뛰어납니다.
      </p>
      <p>
        또한 함수의 내부 로직은 늘 일정하므로, 매개변수만 잘 바꿔주면 의도한 값을 산출할 수
        있습니다.
      </p>

      <h3>유지보수성</h3>
      <CodeBox code={lessonCode.maintainability1} />
      <p>Date 객체의 연, 월, 일을 구하는 로직을 가진 함수를 만들어보았습니다.</p>
      ...
      <p>함수의 로직을 바꾸면 함수를 사용하는 모든 코드들의 아웃풋도 함께 변하겠지요.</p>
      <p>적절한 함수의 사용은 코드의 유지보수성을 크게 증가시킵니다.</p>

      <h3>가독성</h3>
      <p>
        함수 이름이 잘 지어져있으면 함수의 이름만으로도 내부의 로직과 기능을 유추할 수 있습니다.
      </p>
      <CodeBox code={lessonCode.readability} />
      <p>
        사실 우리는 formatDate 함수명만 보고도 저 함수가 어떤 역할을 할 지 유추할 수 있었습니다.
      </p>
      <p>
        (이게 올바른 예제인지는 모르겠네요. 아무튼 결국에는 재사용성, 유지보수성, 가독성 모든 요소를
        잡았습니다.)
      </p>

      <h2>함수의 구성요소</h2>
      <p>이번에는 함수의 구성요소를 살펴볼까요?</p>
      <p>친구에게 인사를 건네는 친절한 함수인 sayHello 함수를 만들어 보겠습니다.</p>
      ...
      <ol>
        <li>함수이름 : 함수 식별자명</li>
        <li>
          파라미터(매개변수) : 함수가 동작할 때 사용할 데이터를 메모리에 저장하는 변수(Variables)
        </li>
        <li>리턴문 : 함수가 실행되었을 때 반환할 값</li>
        <li>아규먼트(인수) : 함수를 호출했을 때 사용할 데이터. () 사이에 추가.</li>
      </ol>
      ...
      <h2>함수 생성 방법</h2>
      <p>자바스크립트의 함수의 생성 방법 중 대표적인 세가지를 알아보도록 하겠습니다.</p>
      <ol>
        <li>
          <b>함수 선언문</b>
        </li>
      ...
      <p>
        함수 선언문으로 함수를 정의하는 방식입니다. 함수를 변수에 할당하지 않고, 함수 자체를
        선언합니다.
      </p>
      <h3>2. 함수 표현식</h3>
      <CodeBox code={lessonCode.functionExpression} />
      <p>
        함수 표현식으로 함수를 정의하는 방식입니다. 함수 자체를 특정 변수에 할당한다는 것이 함수
        선언문과의 차이점이라고 할 수 있겠습니다.
      </p>
      <p>표현식은 값이라고 말씀 드린 적이 있습니다.</p>
      <p>
        함수 표현식에서의 함수는 변수에 할당된 값처럼 동작합니다. 면접 빈출이므로 알아두고 계시면
        좋습니다!
      </p>
      <h3>3. 화살표 함수</h3>
      <CodeBox code={lessonCode.ArrowFunction} />
      <p>
        ES6부터 등장한 화살표 함수입니다. 위의 함수 선언문과 함수 표현식보다 더 간결한 것을 보실 수
        있습니다.
      </p>
      <br></br>
      <p>
        셋 다 함수를 만든다는 측면에서는 같지만, <b>함수 호이스팅</b>이나 <b>this</b>의 유무 등
        내부적인 동작 원리는 약간씩 다릅니다.
      </p>
      <p>일단 키워드만 알아두시고 다음에 필요하실 때 개인적인 공부를 하셔도 좋을 것 같습니다.</p>
      ...
      <p>정의한 함수를 호출(실행)했을때 비로소 내가 만든 함수가 동작을 하기 때문입니다.</p>
      <p>아래 코드를 볼까요?</p>
      <CodeBox code={lessonCode.functionExecute} />
      <p>해리는 제대로 인사를 했는데, 론은 인사를 하지 못했습니다.</p>
      <p>
        해리는 인사하는 기능을 제대로 <b>호출</b>한 데 반해, 론은 인사하는 함수 자체를 <b>할당</b>
        했기 때문입니다.
      </p>
      <br></br>
      <p>
      ...
      <p>
        sayHello의 <b>파라미터</b>는 인사할 친구(friend)였고, 해리가 함수를 호출할 때 사용했던{' '}
        <b>아규먼트</b>는 인사할 대상인 Ron 이었습니다.
      </p>

      <h2>리턴(Return)</h2>
      <p>return문의 역할을 잘 아는 것도 중요합니다.</p>
      <p>
        함수 몸체에 return 문이 없다면, 함수 몸체의 로직을 모두 수행하고 undefined를 리턴합니다.
      </p>
      ...
      <p>
        위의 함수를 실행하면 console.log는 잘 나오는데 변수 helloLog에는 undefined가 할당되어 있는
        것을 보실 수 있습니다.
      </p>
      <p>
      ...
      <p>
        함수 몸체에 return이 있다면, return문에 있는 데이터를 변수에 할당하고 함수가 종료됩니다.
      </p>
      ...
      <p>
        함수를 호출하면 로직이 메모리로 불려와 실행됩니다. 이후 return문을 만나 값을 반환하거나,
        내부 로직이 전부 실행되면 호출한 함수의 로직이 소멸합니다.
      </p>
      <p>이런 이유로 동일한 함수를 여러번 호출했을때도 호출된 각 함수들은 독립적으로 작동합니다.</p>
      <br />
      ...
      <p>
        비동기 함수때문에 많이 당황하셨을 것 같습니다. 비동기는 챕터를 하나 더 빼서라도 말씀드려야
        할 중요한 개념입니다.
      </p>
      <p>
        자바스크립트와 비동기는 뗄레야 뗄 수 없는 관계이기 때문인데요. 이는 다음에 더 자세한
        내용으로 설명을 드리겠습니다.
      </p>

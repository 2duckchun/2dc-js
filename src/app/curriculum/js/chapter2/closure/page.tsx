...
export default function Lecture() {
  return (
    <div className={styles.act_container}>
      ...
      ></ActNavigator>
      <h1>클로저(closure)</h1>
      <p>클로저를 익히면 자바스크립트에 대한 이해도가 대폭 증가한다고 생각합니다.</p>
      <p>언어의 동작원리를 어느정도 알아야지만 클로저를 제대로 이해할 수 있기 때문인데요.</p>
      <p>그래서 바로 클로저 현상부터 설명을 드리는 것 보다는</p>
      <p>클로저를 야기하는 몇가지 자바스크립트의 기초 토대부터 설명을 드리려고 합니다.</p>
      <br />
      <p>
        먼저 <b>일급 객체</b>에 대해 다뤄보겠습니다.
      </p>
      <p>자바스크립트의 들과 함수는 모두 일급 객체인데요.</p>
      <p>일급 객체는 몇가지 특징이 있습니다. 그것들을 잠시 톺아보고 가겠습니다.</p>
      <br />
      <p>
        다음으로 <b>실행 컨텍스트</b>에 대해 다뤄볼 것입니다.
      </p>
      <p>
        실행 컨텍스트는 코드 블록이 실행될때 코드 내부적으로 구동되는 환경이라고 할 수 있습니다.
      </p>
      <p>실행 컨텍스트의 동작 원리를 이해를 하시게 되면</p>
      <p>내가 사용하는 코드들이 어떻게 동작하겠구나를 개념적으로 아실 수 있게 됩니다.</p>
      <br />
      <p>
        실행 컨텍스트에서는 실행 컨텍스트를 이루는 구성 요소인 <b>환경 레코드</b>와{' '}
        <b>렉시컬 환경</b>에 대해 다뤄볼 것입니다.
      </p>
      <p>
      ...
      <p>
        1960년대, 영국의 어떤 학자가 논문 발표회에서 특정 조건을 만족하지 못하는 코드 블록을 이급
        시민같다고 표현한 적이 있는데요.
      </p>
      ...
      <br />
      <p>그리고 객체가 객체를 반환값으로 사용할 수 있는 일급 객체의 특징 때문에</p>
      <p>클로저 현상이 일어난다고도 볼 수 있습니다.</p>
      <p>
        일단은 <b>자바스크립트 함수 = 일급 객체</b> 라고 인지만 하셔도 될 것 같습니다.
      </p>
      <br />

      <h2>실행 컨텍스트</h2>
      <p>실행 컨텍스트는 코드의 실행에 필요한 환경을 지닌 하나의 공간(Context) 입니다.</p>
      <p>별이 존재하려면 우주가 필요합니다. 우주는 별이 존재할 수 있는 환경이지요.</p>
      <p>코드가 별이라고 한다면 실행 컨텍스트는 우주라고 할 수 있겠습니다.</p>
      <p>즉, 실행 컨텍스트는 코드가 동작할 수 있는 환경 그 자체 입니다.</p>
      <br />
      <p>
        실행 컨텍스트는 코드가 일단 실행될 때, 함수가 실행될 때, 블록문이 실행되었을 때 생성되고
      </p>
      <p>해당 컨텍스트의 코드가 다 읽히면 제거됩니다.</p>
      <p>코드 실행 시 맨 처음에 무조건적으로 생기는 컨텍스트를 글로벌 컨텍스트라고 하고,</p>
      <p>
        글로벌 컨텍스트 내부의 코드 진행에 따라 생기는 별도의 컨텍스트를 로컬 컨텍스트라고 합니다.
      </p>
      <br />
      ...
      <ol>
        ...
        <li>
          <b>콜 스택</b> : 엄밀히 말하면 컨텍스트의 구성요소는 아니지만, 컨텍스트의 생명주기에
          관련된 자료구조.
          <p>컨텍스트가 생성되면 콜스택에 쌓이고, 코드가 다 읽히면 콜스택에서 제거됨.</p>
        </li>
      </ol>
      ...
      <p>
        이 때, 코드가 실질적인 동작을 일으키기 전에 코드의 변수들을 한번 쓱 훑어보는 과정이
        있습니다.
      </p>
      <p>그것을 코드 평가 과정이라고 합니다.</p>
      <p>
        평가 과정에서, 변수는 실행 컨텍스트의 <b>환경 레코드</b>라는 부분에 기록이 됩니다.
      </p>
      ...
      <p>전역 컨텍스트는 최상위 컨텍스트이므로 참조할 상위 렉시컬 환경이 없습니다.</p>
      <p>따라서 전역 컨텍스트의 렉시컬 환경은 null이 참조됩니다.</p>
      <p>이렇게 생성된 실행 컨텍스트는 콜 스택이라는 자료구조에 올라가게 됩니다.</p>
      ...
      <p>로컬 실행 컨텍스트도 전역 실행 컨텍스트와 동일한 생성 과정을 거칩니다.</p>
      <p>하지만 변수 등록 과정에서 하나의 차이가 있습니다.</p>
      <p>바로, 함수는 파라미터가 있다는 건데요.</p>
      <p>전역에서 localFn1 함수를 실행할 때 num 파라미터에 50을 할당했습니다.</p>
      <p>따라서, 해당 함수의 실행 컨텍스트의 환경 레코드에는 num = 50 이 추가됩니다.</p>
      <p>이후에 함수 몸체에 있는 변수들과 함수들이 환경 레코드에 등록이 됩니다.</p>
      <p>실행 컨텍스트 생성 과정이 끝나면 다시 코드가 실행되는데요.</p>
      <p>자료 구조상 콜스택의 가장 상단에 있는 코드부터 처리가 되어야 하기 때문에</p>
      ...
      <p>어찌되었든 함수가 호출된 것이므로, 실행 컨텍스트가 하나 더 만들어집니다.</p>
      <p>여기서 중요한 점은 렉시컬 환경의 참조입니다.</p>
      <p>
        함수는 렉시컬 환경을 참조할 때, <b>자신이 선언된 곳의 렉시컬 환경</b>을 참조합니다.
      </p>
      <p>따라서 localFn2는 자신이 선언된 localFn1의 렉시컬 환경을 참조하게 됩니다.</p>
      <br />
      <CodeBox code={lessonCode.localFn2} />
      <p>상단의 코드에서 localFn2만 분리해서 가져온 것입니다.</p>
      <p>localFn2의 환경 레코드가 가지고 있는 변수는 localVar2와 num밖에 없지만,</p>
      <p>자기 자신의 환경 레코드에 없는 변수들은 렉시컬 환경에 의해 참조됩니다.</p>
      <p>따라서 global, localVar1, localVar2, num을 모두 출력할 수 있게 됩니다.</p>
      <p>
        모든 console.log가 실행되면 localFn2의 코드 진행이 끝나므로, 실행 컨텍스트는 콜스택에서
        빠지게 됩니다.
      </p>
      <br />
      <CustomImageWithCaption
      ...
      <p>따라서 console.log를 실행한 후, localFn1의 실행 컨텍스트도 콜스택에서 제거됩니다.</p>
      <br />
      <CustomImageWithCaption
      ...
      <p>전역 컨텍스트도 마지막 console.log를 출력하면 콜스택에서 사라집니다.</p>
      <p>코드 실행이 모두 끝나면 콜스택에서 전역 컨텍스트도 사라지고, 모든 것들이 종료됩니다.</p>
      <br />
      <CustomImageWithCaption
      ...
      <p>코드와 스크린샷을 다시 한번 대조해보시고, 이해하는 시간을 가지시는 것을 추천합니다!</p>
      ...
      <p>그렇다면 아래와 같은 코드도 문법상 허용이 된다는 것을 알 수 있습니다.</p>
      <br />
      <CodeBox code={lessonCode.closure1} />
      <p>위 코드의 동작 원리를 살펴봅시다.</p>
      <p>
        함수가 return을 하게되면 그 함수의 모든 동작은 종료되므로, 함수의 실행 컨텍스트는 콜스택에서
        제거됩니다.
      </p>
      <p>test1과 test2에 innerFn이 할당되는 순간, outerFn은 콜스택에 더이상 남아있지 않습니다.</p>
      <br />
      <p>그런데 innerFn이 선언된 곳은 outerFn입니다.</p>
      <p>즉, innerFn의 렉시컬 환경은 outerFn을 바라보고 있지요.</p>
      <p>innerFn이 outerFn의 렉시컬 환경을 참조하고 있기 때문에</p>
      <p>innerFn의 실행 컨텍스트가 사라져도, 렉시컬 환경의 참조 주소는 남아 있게 됩니다.</p>
      <br />
      <p>
        <b>
          블록의 생명주기가 다했음에도 불구하고, 생명주기가 다한 블록의 렉시컬 환경을 참조할 수 있는
          현상.
        </b>
      </p>
      <p>
      ...
      <p>이 때, 동일한 함수를 여러번 호출했을때도 각 함수들은 독립적으로 동작하므로</p>
      ...
      <p>
        우리는 개발을 하면서 클로저 환경을 의도적으로, 또는 비의도적으로 사용하는 경우가 있습니다.
      </p>
      <p>의도적으로 사용하기는 쉽지만, 비의도적으로 사용되는 것을 파악하기는 어렵습니다.</p>
      ...
      <p>
        웹 페이지에 버튼 2개가 생성됩니다. 각 버튼을 클릭하면 이벤트 핸들러에 의해 버튼의 id가
        console.log로 찍힙니다.
      </p>
      ...
      <p>
        <b>
          addClickHandler의 환경 레코드에 있는 element를 계속 참조할 수 있기 때문에 클로저라고 할 수
          있습니다.
        </b>
      </p>
      <p>
        다만 컨텍스트가 사라졌으므로, 우리가 의도적으로 저 렉시컬 환경을 다시 참조할 방법은
        없습니다.
      </p>
      <p>
        <b>따라서 저 클릭 이벤트 핸들러는 웹을 끌때까지 영원히 메모리를 점유합니다.</b>
      </p>
      <p>비의도적인 클로저의 사용을 늘리게 되면 웹의 속도나 성능이 저하될 수 있습니다.</p>
      <br />
      ...
      <p>내부의 정보를 외부에서 쉽게 참조하거나 수정할 수 없게 하는 것이지요.</p>
      <p>정확히 클로저의 기능과 맞아떨어집니다.</p>
      <CodeBox code={lessonCode.closureUsecase2} />
      <p>함수를 괄호로 감싼 뒤 호출하면 즉시 함수가 호출됩니다.</p>
      <p>이를 즉시 실행 함수라고 하는데요. 여기서 간단히 소개를 드릴까 했습니다.</p>
      <p>위의 Counter 모듈은 오로지 increment, decrement, getCount를 통해서만</p>
      <p>count 변수에 접근할 수 있습니다. 그 외의 방법으로는 접근이 불가능합니다.</p>
      <p>이렇게 의도적으로 클로저 환경을 사용할 수 있습니다.</p>
      <br />

      <h2>클로저의 문제 : 메모리 점유</h2>
      <p>
        클로저 환경은 외부에서 접근할 수 없다는 보안적 이점과 기능을 함수별로 모듈화 할 수 있다는
        장점이 있습니다.
      </p>
      <p>하지만 클로저가 남발될 경우 메모리가 낭비될 가능성이 있습니다.</p>
      <p>
        클로저 환경은 외부에서 직접 접근이 불가능하므로, 애초에 클로저를 설계할 때 메모리를 회수할
        수 있는 방법을 강구해야 합니다.
      </p>
      <p>
        클로저 환경에서 메모리를 회수하는 기능을 <b>클린업(cleanup)</b>이라고 하는데요.
      </p>
      <p>
        클로저 환경을 영원 불멸로 메모리에 남겨둘 것이 아니라면 클린업을 반드시 구현하는 것이
        좋습니다.
      </p>
      <br />
      <h3>클린업 함수</h3>
      <p>
        클로저의 메모리 점유를 막기 위해 <b>함수는 함수를 리턴할 수 있다는 법칙</b>을
        이용해보겠습니다.
      </p>
      <p>
        클로저 환경이 생기려면 렉시컬 환경을 넘겨줄 함수가 동작한 후, 렉시컬 환경을 넘겨주고
        소멸해야 합니다.
      </p>
      <p>
        <b>
          이때 return 값에 렉시컬 환경에 선언된 변수를 초기화하거나, 기능을 없앨 수 있는 함수를
          넘겨줄 수 있습니다.
        </b>
      </p>
      <br />
      <p>비의도적 사용 사례에서 사용한 코드를 재활용해서 클린업 함수를 구현해보겠습니다.</p>
      <p>
        먼저 클린업에 활용할 이벤트 리스너 추가 기능과 이벤트 리스너 제거 기능부터 알아보겠습니다.
      </p>
      <CodeBox code={lessonCode.eventListener} />
      <p>
        <b>EventListener</b>는 특정 이벤트를 감지하면서, 이벤트가 감지되면 콜백함수를 실행시키는
        역할을 합니다.
      </p>
      <p>
        <b>addEventListner</b>는 말 그대로 이벤트 리스너를 달아주는 기능을 합니다.
      </p>
      <p>
      ...
      <p>(setTimeout은 비동기 함수입니다. 비동기와 관련된 내용은 나중에 다뤄볼 예정입니다.)</p>
      <p>클린업을 함수 내부에서 미리 설계를 해놓고 나중에 호출할 수 있게 하면,</p>
      <p>언제든지 클로저 환경에 접근하여 원하는 기능을 이끌어낼 수 있습니다.</p>
      <p>
        <b>
          클로저 환경을 구성할때에는 클린업 함수도 미리 설계가 되어야 한다는 점을 기억하시면 될 것
          같습니다.
        </b>
      </p>
      <br />
      ...
      <p>나중에 다시 돌아오시거나, 다른 책, 다른 글로써 공부를 해보셔도 좋습니다.</p>

...
데이터 타입은 왜 필요할까요? **저는 데이터 타입은 사람을 위해 필요한 것이라고 정의** 합니다.
컴퓨터의 데이터 저장 체계는 0과 1로만 이루어져 있습니다. 메모리에 전기가 들어오면 1, 전기가 없으면 0이죠. (그래서 메모리를 **반도체** 라 부릅니다.)

아무튼 컴퓨터는 0과 1로만 이루어진 데이터, 즉 **비트** 체계로 동작합니다. 하지만 사람이라면 전자 두뇌를 가지지 않는 이상 비트만으로는 의사소통이 불가능 하겠지요. 예시로 **"안녕하세요"** 라는 문자열을 비트로 바꿔보겠습니다.
...
위의 코드 블럭을 보시면 왜 데이터 타입이 사람을 위해 필요한 것인지 이해가 가실 겁니다. 만약 2진수가 더 편하게 느껴지신다면 그... 축하드립니다.

자 그럼 본격적으로 들어가보겠습니다.

먼저 모든 데이터 타입들을 나열해본 뒤 시작하겠습니다. 모든 프로그래밍 언어에는 각자 제공하는 데이터 타입들이 있습니다. 우리가 학습중인 자바스크립트는 기본적으로 7가지 **원시(primitive) 타입** 을 제공합니다.
...
나중에 한번 더 다룰 주제이지만 **참조 타입** 은 모두 **객체(Object)** 입니다. 애초에 자바스크립트 언어가 그렇게 설계가 되어있기 때문인데요. 물론 이 사실만으로 우리에게 큰 의미는 없습니다. 다만 **함수 또한 객체라는 사실** 은 나중에 함수 파트에서 한번 더 설명을 드릴 것 같습니다. 함수가 객체라서 여러모로 편리하거든요.

자. 모든 데이터 타입을 나열해봤습니다. 그럼 순서대로 원시 타입이 무엇인지부터 공부해보도록 하겠습니다.

<h3 id="primitive-type">원시 타입</h3>

원시 타입이라고 하면 고대 석기시대 생각이 나시는 분들이 계실겁니다. 저 또한 그랬습니다. 스타크래프트2 원시 저그 같은 느낌도 들고요. 하여간 프로그래밍에 친숙한 단어는 아닌 느낌이 듭니다.
...
**어디서 파생된 값이 아니며, 고유 데이터를 보유한다 라는 의미** 로 받아들일 수 있겠습니다. 맞습니다. 이러한 성격 때문에 원시 타입이라는 이름이 붙었습니다. 원시 타입에는 어떤 특징이 있는지 코드를 통해 살펴볼까요?
...
당장 이해하시기에는 버거우실지 몰라도 위와 같은 방법으로 이해하시는 편이 미래를 위해서 훨씬 낫습니다. 처음에 잘못 이해를 하게되면 나중에 이 개념에서 연장된 다른 개념의 학습이 많이 어려워지기 때문입니다.

몇몇 퀴즈를 통해 원시 타입과 조금 친해지고 머리도 식히는 시간을 가져보겠습니다.
...
```js
let data = null
let isNull = data ? true : false
let Question = isNull

data = 'javascript'
isNull = data ? true : false
